<?php
// Início do script sem espaços em branco antes do <?php
...
// Obtendo o código do imóvel da URL
$imovel = $_GET['cod'] ?? null;

// Instanciando a classe Imovel
$info = new Imovel;
$detalhesImovel = Imovel::detalhesImovel($imovel);

// Verificação antes de qualquer saída
if (!isset($imovel) || empty($imovel) || empty($detalhesImovel['categoria'])) {
    header("Location: " . BASE_URL);
    exit();
}

// 1) Convert cod -> numeric id
$propertyId = Imovel::getIdByCod($imovel);

// 2) Fetch pontos próximos
$pontosProximos = Imoveis::getPontosProximosByImovelId($propertyId);

// Configuração do cookie antes de qualquer saída HTML
$updateViews = false;
...
<head>
    ...
    $page_title = ucfirst($detalhesImovel['categoria']) . " por {$preis} em {$detalhesImovel['cidade']}, {$detalhesImovel['bairro']}";
            ...
            <div class="container">
                        ...
                        <div class="property-single-gallery-info">
                            <div class="property--info clearfix">
                                ...
                                <div class="pull-right">
                                    ...
                                    <?php
                                    // Retrieve the linked property record using the current property ID.
                                    $linked = \Source\Imoveis::getLinkedImovel($propertyId);

                                    // Initialize an empty variable for the button HTML.
                                    $linkButtonHTML = '';

                                    // Check if a linked record exists.
                                    if ($linked) {
                                        // Normalize type for comparison using the detailed data array.
                                        $currentType = strtolower($detalhesImovel['tipo']);

                                        if ($currentType === 'vendas' && !empty($linked->id_aluguel)) {
                                            // For a property for sale, if a linked rental property exists, show the button.
                                            $linkButtonHTML = '<a href="imovel.php?cod=' . htmlspecialchars($linked->id_aluguel) . '" class="btn btn--primary">Disponível para locação</a>';
                                        } elseif ($currentType === 'aluguel' && !empty($linked->id_venda)) {
                                            // For a property for rent, if a linked sale property exists, show the button.
                                            $linkButtonHTML = '<a href="imovel.php?cod=' . htmlspecialchars($linked->id_venda) . '" class="btn btn--primary">Disponível para venda</a>';
                                        }
                                    }

                                    // Then, output the button (if any):
                                    echo $linkButtonHTML;
                                    ?>
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-4">
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <?php if (!empty($pontosProximos)): ?>
                                    <div class="widget widget-nearby-points">
                                        <div class="widget--title">
                                            <h5>Pontos mais Próximos</h5>
                                        </div>
                                        <div class="widget--content">

                                            <ul class="pontos-proximos-list">
                                                <?php foreach ($pontosProximos as $pt): ?>
                                                    <li>
                                                        <!-- Icon could be a FontAwesome class if stored in $pt->icone -->
                                                        <i class="fa-solid <?= htmlspecialchars($pt->icone) ?>"></i>
                                                        <?= htmlspecialchars($pt->estabelecimento) ?>
                                                    </li>
                                                <?php endforeach; ?>
                                            </ul>
                                        <?php endif; ?>
